...
public class ProductTest {
    ...
    public void 상품_생성_성공_테스트() {
        Product product = new Product("콜라", 1000, 10, "탄산2+1",10);
        assertEquals("콜라", product.getName());
        assertEquals(1000, product.getPrice());
        assertEquals(10, product.getRegularQuantity());
    ...
    public void 프로모션_없는_경우_생성_성공_테스트() {
        Product product = new Product("사이다", 1000, 8, null,10);
    ...
    public void 가격_음수_생성_실패_테스트() {
        Exception exception = assertThrows(IllegalArgumentException.class, () -> {
            new Product("콜라", -100, 10, "탄산2+1",10);
        });
    ...
    public void 재고_음수_생성_실패_테스트() {
        Exception exception = assertThrows(IllegalArgumentException.class, () -> {
            new Product("콜라", 1000, -5, "탄산2+1",10);
        });
    ...
    public void 재고_감소_성공_테스트() {
        ...
        assertEquals(7, product.getRegularQuantity());
    }

    @DisplayName("재고를 초과한 감소 실패 테스트")
    @Test
    public void 재고_초과_감소_실패_테스트() {
        Product product = new Product("콜라", 1000, 5, "탄산2+1",10);
        Exception exception = assertThrows(IllegalArgumentException.class, () -> {
    ...
    public void 재고_정상_증가_테스트() {
        ...
        assertEquals(8, product.getRegularQuantity());
    }
}